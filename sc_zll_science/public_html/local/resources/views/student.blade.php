    ...
    <div id="main" class="w950">
        <div class="right" style="width: 640px;">
            ...
            <div class="box2" style="margin-bottom: 5px;">
                ...
                <div class="jyborder">
                    <ol class="line03">
                        <li class="lin02bg"><u>订单号</u><em>学员性别</em><i>求教科目 </i><b style="width: 18%">学员要求</b><em style="width: 22%">授课区域</em><i>发布时间</i><em>支付</em></li>
                        
                                @foreach($stu as $t)
                                <li class="">
                                    <u style="width:5%">{{$t->onum}}</u><em>{{$t->gender}}</em><i>{{$t->subject}}</i>
                                    <b style="width: 20%">{{$t->requirement}}</b><em style="width: 22%">{{$t->address}}</em><i>{{date('Y-m-d',$t->pubtime)}}</i>
                                    @if(Request::user()=='')
                                    <em><a href="{{url('/auth/login')}}" target="_blank" title="购买信息" >登陆支付</a></em>
                                </li>
                                @elseif(Request::user()->role == 1)
                                <em>
                                    <form action="{{url('payment')}}" method="post">
                                    {!!csrf_field()!!}
                                    <input type="hidden"  name="onum" value="{{$t->onum}}"/>
                                    @if($t->status == 0)
